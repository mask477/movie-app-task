...
export default function Movie({ id, poster, title, year }: Movie) {
  ...
  return (
    <Link href={`/movies/${id}/edit`} prefetch>
      <Card className="flex flex-col gap-4 p-2 p-0 sm:p-2 hover:bg-foreground-focus">
        <div className="relative aspect-[266/400] w-full">
          ...
          <Image
            ...
            alt={title}
            className={cn([
              'w-full h-full sm:rounded-lg object-cover transition-opacity duration-1000',
